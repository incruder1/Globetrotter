...
import { Share2, Globe, ArrowRight, Trophy, MapPin, Smile, Frown, Compass, Info, Clock, Award } from 'lucide-react';
...
const GamePage = () => {
  ...
  const fetchRandomDestination = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:3001/api/destinations/random');
  ...
  const handleTimeout = async () => {
    ...
    const randomFunFact = destination.funFacts[Math.floor(Math.random() * destination.funFacts.length)];
        ...
        {user && (
          <div className="flex flex-wrap items-center justify-center sm:justify-end gap-3">
            ...
            <div className="flex items-center bg-white bg-opacity-20 backdrop-blur-sm rounded-full px-4 py-2 text-sm">
              ...
              <span className="font-medium">{user.score.correct} / {questionCount}</span>
            </div>
            <button 
          ...
          <div className="max-w-2xl w-full bg-white rounded-2xl shadow-2xl overflow-hidden text-gray-800">
            <div className="p-8 text-center">
              ...
              <p className="text-xl mb-6">
                You scored <span className="font-bold text-blue-600">{user?.score.correct}</span> out of {MAX_QUESTIONS}!
              </p>
            ...
            {destination && (
              <div className="relative">
                ...
                <div 
                  ...
                  style={{
                    backgroundImage: "url('https://images.unsplash.com/photo-1526778548025-fa2f459cd5ce?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1920&q=80')",
                ...
                <div className="p-6 sm:p-8 relative">
                  <div className="mb-8">
                    ...
                    <div className="bg-blue-50 rounded-xl p-5 mb-6 border-l-4 border-blue-500 shadow-inner">
                      {destination.clues.map((clue, index) => (
                        <div key={index} className="mb-3 last:mb-0 flex">
                          <MapPin className="text-blue-500 mr-2 flex-shrink-0 mt-1" size={18} />
                    ...
                    <h3 className="text-lg font-semibold text-gray-700 mb-3">Select the correct destination:</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                      {destination.options.map((option) => (
                        ...
                        >
                          ...
                          {option.id === destination.id && result !== null && selectedOption !== option.id && (
                            <Smile className="text-green-500" size={20} />
                          )}
                        </button>
                      ))}
                    ...
                    >
                      <div className="flex items-start">
                        <div className={`p-3 rounded-full ${
                          result === 'correct' ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500'
                        } mr-4`}>
                          {result === 'correct' ? (
                            <Smile size={32} />
                          ) : (
                        ...
                        <div>
                          ...
                          {result !== 'correct' && (
                            <p className="text-gray-700 mb-2">
                              The correct answer was <span className="font-semibold">{destination.location}</span> in {correctLocation}.
                            </p>
                          )}
                          <div className="flex items-start">
                            <Info size={18} className="text-blue-500 mr-2 flex-shrink-0 mt-1" />
                            <p className="text-gray-700">
                  ...
                  {result && (
                    <div className="flex justify-center">
                      ...
                      >
                        Next Destination <ArrowRight size={20} className="ml-2" />
                      </button>
                    </div>
                  )}
                </div>
